...
class Block:
    def __init__(self, col, rect, speed = 0):
        ...
        self.rect = rect
        self.speed = speed
        # 90이면 위로, 270이면 공이 아래로 -45~45편차로
        self.dir = random.randint(-45, 45) + 90    

    def move(self): # 볼 무브
        # 볼이 움직이는 x축 값을 계속 계산하려면 x은 dir 값을 라디언으로 변환 후 코사인처리
        self.rect.centerx += math.cos(math.radians(self.dir)) * self.speed
        # 볼이 움직이는 y축 값을 계속 계산하려면 y은 dir 값을 라디언으로 변환 후 사인처리
        self.rect.centery -= math.sin(math.radians(self.dir)) * self.speed

    def draw_E(self): # 공을 circle이 아니라 ellips로 생성
...
def main():
    ...
    while True:
        ...
        if is_game_start == False:
            ...
            elif BALL.rect.centery < 10: # 게임화면 천장에 부딪히면 반사 ## 반지름 계산까지 10
                BALL.dir = -BALL.dir

            # 게임 클리어, 종료 로직
            if len(BLOCK) == 0: # 볼로 블럭 다 없앴음
                Surface.blit(M_CLEAR, ((SCREEN_WIDTH / 2) - (240 / 2)
                                    ,(SCREEN_HEIGHT / 2) - (50 / 2)))
            if BALL.rect.centery > 800:
                Surface.blit(M_FAIL, ((SCREEN_WIDTH / 2) - (240 / 2)
                                    ,(SCREEN_HEIGHT / 2) - (50 / 2)))
                is_game_start = False # 게임 종료 후 재시작
